	...
	if (threadIdx.x == 0){
		...
		while (subS1 != setS){
			...
			if (bitS2 == (bitS - 1)){
				...
				printf("Cost for %d is %d, and for %d is %d", subS1, c1, subS2, c2);
				if (first_run){
...
int main(){
	...
	for (int j = 1; j <= bitNumber; j++) {
		...
		if (currBitSet == 1){
			...
			printf("current table No: %d, value is %d shoud be %d \n", currBitSet * 3, dp_table[currBitSet * 3], sql_input[table]);
...
//Allocating needed memory 
				...
				cudaMemcpy(dev_table, dp_table, dp_table_size, cudaMemcpyHostToDevice);
//Launching the kernel
				subSetKernel <<<1, coeff+1>>>(dev_table, dev_sets, coeff);
//Copying back to host
				cudaMemcpy(dp_table, dev_table, dp_table_size, cudaMemcpyDeviceToHost);
